...
local REQUIRED_KEY = "ITALLO-2026"
...
keyBox.Name = "KeyBox"
keyBox.Size = UDim2.fromOffset(380, 285)
...
keyScale.Name = "MobileScale"
keyScale.Scale = math.clamp(math.min(camera.ViewportSize.X / 420, camera.ViewportSize.Y / 620), 0.62, 0.86)
keyScale.Parent = keyBox
...
keyStatus.Position = UDim2.fromOffset(20, 188)
...
local generateKeyButton = Instance.new("TextButton")
generateKeyButton.Name = "GenerateKeyButton"
generateKeyButton.Size = UDim2.new(1, -40, 0, 38)
generateKeyButton.Position = UDim2.fromOffset(20, 218)
generateKeyButton.BackgroundColor3 = THEME.surface
generateKeyButton.BorderSizePixel = 0
generateKeyButton.Font = Enum.Font.GothamBold
generateKeyButton.TextSize = 13
generateKeyButton.TextColor3 = THEME.text
generateKeyButton.Text = "GERAR KEY"
generateKeyButton.AutoButtonColor = false
generateKeyButton.ZIndex = 102
generateKeyButton.Parent = keyBox

do
	local c = Instance.new("UICorner")
	c.CornerRadius = UDim.new(0, 7)
	c.Parent = generateKeyButton
	local st = Instance.new("UIStroke")
	st.Color = THEME.border
	st.Thickness = 1
	st.Transparency = 0.15
	st.Parent = generateKeyButton
end
...
headerStatus.Size = UDim2.fromOffset(92, 20)
headerStatus.Position = UDim2.new(1, -145, 0, 11)
...
local espTab = makeButton(tabsBar, "ESP", UDim2.fromOffset(54, 32), UDim2.fromOffset(6, 5))
local aimTab = makeButton(tabsBar, "AIM", UDim2.fromOffset(54, 32), UDim2.fromOffset(64, 5))
local flightTab = makeButton(tabsBar, "VOAR", UDim2.fromOffset(54, 32), UDim2.fromOffset(122, 5))
local tpTab = makeButton(tabsBar, "TP", UDim2.fromOffset(54, 32), UDim2.fromOffset(180, 5))
local godTab = makeButton(tabsBar, "MODS", UDim2.fromOffset(64, 32), UDim2.fromOffset(238, 5))
local fpsTab = makeButton(tabsBar, "FPS", UDim2.fromOffset(54, 32), UDim2.fromOffset(306, 5))
...
local openButton = makeButton(screenGui, "✦ ITALLO7", UDim2.fromOffset(104, 38), UDim2.fromOffset(20, 100))
...
-- Permite mover o botão enquanto o painel estiver fechado.
...
local function togglePanel()
	...
	if not panelOpen then
		-- Ao fechar, o botão fica na mesma linha do título "Itallo7".
		openButton.Position = UDim2.fromOffset(title.AbsolutePosition.X, title.AbsolutePosition.Y)
	end
	panel.Visible = panelOpen
	openButton.Visible = not panelOpen
end
...
local function generateLocalKey()
	-- Geração local: adequada para teste/uso simples.
	-- Para segurança real, a geração/validação deve ficar no servidor.
	...
	for partIndex = 1, 3 do
		...
		for i = 1, 4 do
			...
			part[i] = chars:sub(index, index)
		end
		parts[partIndex] = table.concat(part)
	end

	return table.concat(parts, "-")
end

generateKeyButton.Activated:Connect(function()
	local generated = generateLocalKey()

	-- A key gerada passa a ser a key válida desta sessão.
	REQUIRED_KEY = generated
	keyInput.Text = generated
	keyStatus.Text = "Key gerada! Toque em ENTRAR."
	keyStatus.TextColor3 = THEME.cyan

	generateKeyButton.Text = "KEY GERADA ✓"
	TweenService:Create(
		generateKeyButton,
		TweenInfo.new(0.15),
	...
	task.delay(1.2, function()
		if generateKeyButton and generateKeyButton.Parent then
			generateKeyButton.Text = "GERAR KEY"
			TweenService:Create(
				generateKeyButton,
				TweenInfo.new(0.15),
